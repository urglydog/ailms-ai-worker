...
def _build_direct_prompt(numbered: dict[str, str], source_lang: str, target_lang: str) -> str:
    ...
    return f"""## Role
Ban la bien dich vien chuyen nghiep, dich {source_lang} sang {target_lang} cho video bai giang.

## Task
Dich TUNG dong duoi day tu {source_lang} sang {target_lang}, dich SAT NGHIA, khong bo sot
...
def _build_adapt_prompt(direct: dict[str, str], durations_sec: dict[str, float], target_lang: str) -> str:
    ...
    return f"""## Role
Ban la bien tap vien phu de/loi thoai cho video giao duc bang {target_lang}.
...
async def resummarize(text: str, target_lang: str) -> str:
    ...
    """
    prompt = f"""## Role
Ban la bien tap vien rut gon loi thoai cho video giao duc bang {target_lang}.
